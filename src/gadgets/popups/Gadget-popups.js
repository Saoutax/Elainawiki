...
$(() => {
    ...
    function isCorrectModifier(a, evt) {
        ...
        const action = getValueOf('popupModifierAction');
        return (
            (action == 'enable' && modifierPressed(evt)) ||
    ...
    function nonsimplePopupContent(a, article) {
        ...
        if (referenceElement) {
            ...
            if (article.namespaceId() == pg.nsCategoryId && shouldShow(a, 'popupCategoryMembers')) {
                ...
                (article.namespaceId() == pg.nsUserId ||
                    article.namespaceId() == pg.nsUsertalkId) &&
    ...
    pg.structures.fancy.popupOtherLinks = function (x) {
        ...
        const normal =
            '<<whatLinksHere|shortcut=l|links here>>*<<relatedChanges|shortcut=r|related>>';
        return navlinkStringToHTML(
    ...
    pg.structures.menus.popupTopLinks = function (x, shorter) {
        ...
        const watch =
            '<menurow><<unwatch|unwatchShort>>|<<watch|shortcut=w|watchThingy>></menurow>';
        const protect =
        ...
        const nullPurge =
            '<menurow><<nullEdit|shortcut=n|null edit>>|<<purge|shortcut=P>></menurow>';
    ...
    Insta.convert = function (wiki) {
        ...
        function parse_inline_images(str) {
            ...
            while ((start = str.indexOf('[[', substart)) != -1) {
                if (
                    str
                        .substr(start + 2)
                        .match(RegExp('^(Image|File|' + Insta.conf.locale.image + '):', 'i'))
                ) {
                    loop = true;
                    substart = start;
                    do {
        ...
        function parse_inline_wiki(str) {
            ...
            str = str
                ...
                .replace(/\[http:\/\/(.*?)\]/g, ($0, $1) =>
                    f("<a class='external' href='http://?'>[#]</a>", htmlescape_attr($1)),
                )
    ...
    function popupFilterCountLinks(data) {
        ...
        return (
            String(num) + '&nbsp;' + (num != 1 ? popupString('wikiLinks') : popupString('wikiLink'))
        );
    }
    ...
    function popupFilterCountCategories(data) {
        ...
        return (
            String(num) +
            '&nbsp;' +
            (num != 1 ? popupString('categories') : popupString('category'))
        );
    }
    ...
    function getPageInfo(data, download) {
        ...
        const pageInfoArray = runPopupFilters(
            popupFilters.concat(extraPopupFilters),
            data,
            download,
        );
    ...
    function listLinks(wikitext, oldTarget, titleToEdit) {
        ...
        for (let i = 1; i < splitted.length; i = i + 3) {
            if (
                ...
                ret.push(
                    retargetDab(splitted[i], oldTarget, friendlyCurrentArticleName, titleToEdit),
                );
            } /* if */
        } /* for loop */
        ...
        if (wikPos) {
            const wikTarget =
                'wiktionary:' + friendlyCurrentArticleName.replace(/^(.+)\s+[(][^)]+[)]\s*$/, '$1');
    ...
    function fillEmptySpans(args) {
        ...
        for (let i = 0; i < numspans; ++i) {
            ...
            if (
                ...
                (spans[i].indexOf('popupTopLinks') === 0 ||
                    spans[i].indexOf('popupRedirTopLinks') === 0)
            ) {
    ...
    Previewmaker.prototype.mopup = function () {
        ...
        // this'll probably do
        this.data = this.data.replace(/^__[A-Z_]*__ *$/gim, '');
    };
    ...
    Previewmaker.prototype.firstBit = function () {
        ...
        const notSentenceEnds =
            /([^.][a-z][.] *[a-z]|etc|sic|Dr|Mr|Mrs|Ms|St|no|op|cit|\[[^\]]*|\s[A-Zvclm])$/i;
        ...
        switch (queryType) {
            ...
            case 'imagepagepreview':
                ...
                url +=
                    'titles=' +
                    art +
                    '&prop=revisions|imageinfo&rvslots=main&rvprop=content' +
                    trail;
        ...
        const callback = function (d) {
            ...
            if (queryType === 'userinfo') {
                ...
                fetchUserGroupNames(d.data).then(() => {
                    showAPIPreview(
                        queryType,
                        htmlGenerator(article, d, navpop),
                        navpop.idNumber,
                        navpop,
                        d,
                    );
    ...
    function getLocales() {
        if (!pg.user.locales) {
            ...
            pg.user.locales = Intl.DateTimeFormat.supportedLocalesOf([
                userLanguage,
                navigator.language,
            ]);
        }
    ...
    function editPreviewTable(article, h, reallyContribs) {
        ...
        for (let i = 0; i < h.length; ++i) {
            ...
            if (thisDay) {
                html.push(
                    '<tr><td colspan=3><span class="popup_history_date">' +
                        thisDay +
                        '</span></td></tr>',
                );
            }
            ...
            const c =
                h[i].comment ||
                (typeof h[i].slots !== 'undefined' ? h[i].slots.main.content : null);
            if (c) {
    ...
    function fetchUserGroupNames(userinfoResponse) {
        ...
        if (user.groups) {
            user.groups.forEach(groupName => {
                if (
                    ['*', 'user', 'autoconfirmed', 'extendedconfirmed', 'named'].indexOf(
                        groupName,
                    ) === -1
                ) {
    ...
    function APIrevisionPreviewHTML(article, download) {
        try {
            ...
            if (page.pageprops.wikibase_item) {
                ...
                download.wikibaseRepo =
                    jsObj.query.wikibase.repo.url.base + jsObj.query.wikibase.repo.url.articlepath;
            }
        } catch (someError) {
        ...
        try {
            ...
            if (alt) {
                ret =
                    ret +
                    '<hr /><b>' +
                    popupString('Alt text:') +
                    '</b> ' +
                    pg.escapeQuotesHTML(alt);
            }
            if (typeof content === 'string') {
    ...
    function APIuserInfoPreviewHTML(article, download) {
        ...
        if (user) {
            ...
            if (globaluserinfo && ('locked' in globaluserinfo || 'hidden' in globaluserinfo)) {
                ...
                for (
                    let i = 0;
                    globaluserinfo.unattached && i < globaluserinfo.unattached.length;
                    i++
                ) {
        ...
        if (queryobj.usercontribs && queryobj.usercontribs.length) {
            ret.push(
                popupString('last edit on ') +
                    formattedDate(new Date(queryobj.usercontribs[0].timestamp)),
            );
        }
        ...
        if (queryobj.blocks) {
            ...
            for (let l = 0; l < queryobj.blocks.length; l++) {
                let rbstr =
                    queryobj.blocks[l].rangestart === queryobj.blocks[l].rangeend
                        ? 'BLOCK'
                        : 'RANGEBLOCK';
    ...
    function setSiteInfo() {
        ...
        pg.wiki.wikimedia =
            /(wiki([pm]edia|source|books|news|quote|versity|species|voyage|data)|metawiki|wiktionary|mediawiki)[.]org/.test(
                pg.wiki.hostname,
            );
    ...
    function setMainRegex() {
        ...
        let preTitles =
            ...
            literalizeRegex(mw.config.get('wgScriptPath')) +
    ...
    function expandConditionalNavlinkString(s, article, z, recursionCount) {
        ...
        for (let i = 1; i < splitted.length; i = i + numParens + 1) {
            ...
            switch (testString) {
                ...
                case 'mainspace_en':
                    testResult =
                        isInMainNamespace(article) && pg.wiki.hostname == 'en.wikipedia.org';
                    break;
                case 'wikimedia':
    ...
    navlinkTag.prototype.getPrintFunction = function () {
        ...
        switch (this.id) {
            ...
            case 'sinceMe':
                ...
                break;
            case 'togglePreviews':
                this.text = popupString(
                    pg.option.simplePopups ? 'enable previews' : 'disable previews',
                );
            /* fall through */
            case 'disablePopups':
            case 'purgePopups':
    ...
    function wikiLink(l) {
        ...
        if (
            !(
                typeof l.article == typeof {} &&
                typeof l.action == typeof '' &&
                typeof l.text == typeof ''
            )
        ) {
            return null;
        }
        ...
        if (hint) {
            ...
            hint =
                safeDecodeURI(l.article + '&action=' + l.action) + l.oldid
                    ? '&oldid=' + l.oldid
                    : '';
        }
    ...
    function processDiffSinceMyEdit(info, stuff) {
        ...
        if (info.myLastEdit.index === 0) {
            alert(
                tprintf('%s seems to be the last editor to the page %s', [
                    info.userName,
                    friendlyName,
                ]),
            );
    ...
    function editCounterLink(l) {
        ...
        let url;
        const defaultToolUrl =
            'https://xtools.wmflabs.org/ec?user=$1&project=$2.$3&uselang=' +
    ...
    function globalSearchLink(l) {
        ...
        const base =
            'https://global-search.toolforge.org/?uselang=' +
            mw.config.get('wgUserLanguage') +
            '&q=';
    ...
    function finishProcessHistory(edits, userName) {
        ...
        for (let i = 0; i < edits.length; ++i) {
            if (
                typeof histInfo.myLastEdit === 'undefined' &&
                userName &&
                edits[i].editor == userName
            ) {
                histInfo.myLastEdit = {
                    index: i,
                    oldid: edits[i].oldid,
            ...
            if (
                typeof histInfo.firstNewEditor === 'undefined' &&
                edits[i].editor != edits[0].editor
            ) {
    ...
    function setOptions() {
        ...
        newOption(
            ...
            popupString('defaultpopupRevertToPreviousSummary'),
        );
    ...
    pg.string = {
        ...
        /////////////////////////////////////
        actions: '动作', ///// view articles and view talk
        popupsMenu: 'Popups',
        ...
        new: '新',
        'new topic': '新话题',
        newSectionHint: '在%s增加新的评论话题',
        'null edit': '空编辑',
        nullEditHint: '对%s提交一次空编辑',
        hist: '历史', ///// history, diffs, editors, related
        history: '历史',
        historyHint: '%s的修订历史',
        'History preview failed': '历史预览失败',
        last: '最近', // For labelling the previous revision in history pages; the key is "last" for backwards compatibility
        lastEdit: '最近更改',
        ...
        editOld: '编辑旧版', ///// edit old version, or revert
        rv: '回退',
        revert: '回退',
        revertHint: '回退到%s',
        defaultpopupReviewedSummary: '标记从版本%s到%s间的编辑为已巡查 //Popups',
        defaultpopupRedlinkSummary: '取消到空页面[[%s]]的链接 //Popups',
        defaultpopupFixDabsSummary: '消歧义[[%s]]到[[%s]] //Popups',
        defaultpopupFixRedirsSummary: '忽略从[[%s]]到[[%s]]的重定向 //Popups',
        defaultpopupExtendedRevertSummary: '回退到$2在$1时制作的修订版本$3 //Popus',
        defaultpopupRevertToPreviousSummary: '回退到修订版本%s的上一个版本 //Popus',
        defaultpopupRevertSummary: '回退到修订版本%s //Popus',
        defaultpopupQueriedRevertToPreviousSummary:
            '回退到修订$1（由$3在$2时制作）的上一个修订版本 //Popus',
        defaultpopupQueriedRevertSummary: '回退到$3在$2时制作的修订版本$1 //Popus',
        defaultpopupRmDabLinkSummary: '移除到消歧义页[[%s]]的链接 //Popus',
        ...
        space: '空间', // short form for userSpace link
        PrefixIndexHint: '显示%s的用户空间',
        count: '统计', ///// contributions, log
        'edit counter': '编辑次数',
        editCounterLinkHint: '%s的编辑次数',
        contribs: '贡献',
        contributions: '贡献',
        deletedContribs: '已删除的贡献',
        DeletedcontributionsHint: '用户%s的被删除编辑次数',
        ContributionsHint: '%s的用户贡献',
        log: '日志',
        'user log': '用户日志',
        userLogHint: '显示%s的用户日志',
        arin: 'ARIN查询', ///// ARIN lookup, block user or IP
        ...
        'Invalid or IP user': '错误的用户名或IP用户',
        'Not a registered username': '非已注册的用户',
        BLOCKED: '被封禁',
        'Has blocks': '被部分封禁',
        ...
        'Failed to get revision information, please edit manually.\n\n':
            '获取修订版本信息失败，请手动修改。\n\n',
        'The %s button has been automatically clicked. Please wait for the next page to load.':
            '按钮%s已被自动点击，请等待下一个页面加载。',
        'Could not find button %s. Please check the settings in your javascript file.':
            '找不到按钮%s，请检查您JavaScript文件中的设置。',
        /////////////////////////////////////
        // Popups setup
        /////////////////////////////////////
