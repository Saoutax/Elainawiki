...
window.wgUVS = (hans, hant, cn, tw, hk, sg, zh, mo, my) =>
...
mw.log.deprecate(
    window,
    'addPortletLink',
    (...args) => mw.util.addPortletLink.bind(mw.util)(...args),
...
mw.log.deprecate(
    window,
    'getURLParamValue',
    (...args) => mw.util.getParamValue.bind(mw.util)(...args),
...
mw.log.deprecate(
    ...
    (page, ready) => libCachedCode.injectCachedCode(page, 'script').then(ready),
    'Use `await libCachedCode.injectCachedCode(page, "script")` instead',
);
...
window.wgGetEditRequestPreload = (
    pageName = mw.config.get('wgPageName'),
    basePageName = window.libGetPageNames().basePageName,
) =>
    basePageName !== false && /^MediaWiki:Conversiontable\/zh-[a-z]+$/.test(pageName)
        ? `Template:编辑请求/${basePageName}`
        : 'Template:编辑请求/comment';
