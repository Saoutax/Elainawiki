...
(() => {
    ...
    window.oouiDialog = Object.fromEntries(
        ['alert', 'confirm', 'prompt'].map(method => [
            method,
            async (textORjQuery, _option) => {
                ...
                const sizeNames = Object.keys(sizes)
                    ...
                    .sort((a, b) => sizes[a].width - sizes[b].width); // ["small", "medium", "large", "larger"]
                ...
                if (option.allowFullscreen !== true) {
                    ...
                    if (acceptableSize.length > 0) {
                        setup.size = acceptableSize.includes(option.size)
                            ? option.size
                ...
                try {
                    ...
                    while (Number.MAX_SAFE_INTEGER > Number.MIN_SAFE_INTEGER) {
                        result = await OO.ui[method](
                            textORjQuery instanceof $ ? textORjQuery : $('<p>').html(textORjQuery),
                            {
                                title: '萌娘百科提醒您',
                                ...option,
                                ...setup,
