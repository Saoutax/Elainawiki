(async () => {
    ...
    const lua_getString = (str) => {
        ...
        if (
            first_char === trim_check.charAt(trim_check.length - 1) &&
                    ...
                    for (const prop in v) {
                        if (Object.hasOwnProperty.bind(v)(prop)) {
                            const testArr_k = prop,
                                testArr_v = v[prop];
                            if (
                                new RegExp(wikiTextKey).exec(testArr_k) &&
                                typeof testArr_v === 'string'
                            ) {
    ...
    const noticeHTML = {
        loading: `<div class="mw-_addText-preview-loading"><div class="quotebox" style="margin: auto; width: 50%; padding: 6px; border: 1px solid #aaa; font-size: 88%; background-color: #F9F9F9;"><div class="mw-_addText-preview-loading-content" style="background-color: #F9F9F9; color: black; text-align: center; font-size: larger;"><img src="https://static.wikitide.net/elainawiki/4/42/Loading.gif" decoding="async" data-file-width="64" data-file-height="64" style="width: 32px; height: 32px;"> ${wgULS('预览加载中...', '預覽載入中...')} </div></div></div>`,
        fail: `<img src="https://static.wikitide.net/elainawiki/1/1a/Mbox-caution.svg" decoding="async" data-file-width="48" data-file-height="48" width="32" height="32">${wgULS('预览加载失败', '預覽載入失敗')}`,
    };
    ...
    const mwGetFromNowRevision = async () => {
        try {
            ...
            const pageContent = getCleanText(
                `${document.querySelector('#mw-clearyourcache') ? '{{#invoke:SpecialWikitext/Template|int|clearyourcache}}' : ''}${parsedHTML}`,
            );
            if (pageContent) {
    ...
    (() => {
        if (
            ...
            if (document.querySelector('.previewnote')) {
                ...
                !document.querySelector('.mw-_addText-content') &&
                mwConfigIfMatchInLowerCase('wgAction', 'view')
            ) {
                if (!ifNeedPreview()) {
                    return;
                }
                ...
                document.querySelector('#mw-revision-info') &&
                mwConfigIfMatchInLowerCase('wgAction', 'view')
            ) {
                if (!document.querySelector('#wpTextbox1')) {
            ...
            if (
                document.querySelector('#wpTextbox1') &&
                document.querySelector('table.diff') &&
                !document.querySelector('.previewnote') &&
                !mwConfigIfMatchInLowerCase('wgAction', 'view')
            ) {
            ...
            if (document.querySelector('.specialWikitextJSON, pre, .mw-json')) {
                ...
                const tryAddWiki =
                    getCleanText(lua_getJSONwikitext($tryGetWiki)) ||
                    getCleanText(lua_getCSSwikitext($tryGetWiki));
                if (tryAddWiki) {
                    addLoadingNotice();
            ...
            !document.querySelector('.mw-editnotice') &&
        ...
        try {
            ...
            for (let i = 0; i <= wikitextPackages.length; i++) {
                $(`.special-wikitext-preview-testcase[data-preview-id=${i}]`)
                    ...
                    .before(
                        parsedNodes.find(
                            `.special-wikitext-preview-testcase-container > .special-wikitext-preview-testcase[data-preview-id=${i}]`,
                        ),
                    );
            }
            removeLoadingNotice();
