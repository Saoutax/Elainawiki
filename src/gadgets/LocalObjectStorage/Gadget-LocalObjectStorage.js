...
(() => {
    ...
    class LocalObjectStorage {
        static plugins = {
            transformers: {
                get list() {
                    return externalTransformers.map(transformer =>
                        Object.assign(Object.create(null), transformer),
                    );
                },
        ...
        constructor(prefix = '', { expires } = {}) {
            ...
            if (prefix.includes('/')) {
                throw new Error(
                    `LocalObjectStorage can't accept prefix "${prefix}" including "/".`,
                );
            }
        ...
        #calcExpiresAtPrefix = expires => {
            ...
            return `${LocalObjectStorage.#EXPIRES_PREFIX}${moment()
